...
class CopsAndRobbersApp:
    ...
    def _perform_setup_action(self, action: str) -> None:
        if action == "n_minus":
            ...
            self.setup_bot_depth = clamp(self.setup_bot_depth - 1, 1, 6)
    ...
    def _setup_steppers(self) -> list[Stepper]:
        return [
            ...
            Stepper("Bot depth:", self.setup_bot_depth, "depth_minus", "depth_plus", 1, 6),
        ]
    ...
    def _stepper_y(self, label: str) -> int:
        lookup = {"Vertices n:": 135, "Edges m:": 189, "Rounds T:": 243, "Bot depth:": 297}
        return lookup[label]
    ...
    def _apply_bot_turn_if_ready(self) -> None:
        ...
        destination = bot.choose_move(self.state, legal_moves)
        self.rules.apply_move(self.state, destination)
        self.last_bot_action_ms = now

    def _advance_bot_placement(self) -> None:
        if self.graph is None:
            return
        if self.selected_cop_position is None and self._is_bot_role(PlayerRole.COP):
            self.selected_cop_position = self._choose_bot_start_position(PlayerRole.COP, blocked=None)

        if self.selected_cop_position is not None and self._is_bot_role(PlayerRole.ROBBER):
            robber_position = self._choose_bot_start_position(
                PlayerRole.ROBBER,
                blocked=self.selected_cop_position,
            )
            self._start_on_selected_positions(self.selected_cop_position, robber_position)

    def _choose_bot_start_position(self, role: PlayerRole, blocked: int | None) -> int:
        if self.graph is None:
            raise RuntimeError("cannot choose a bot start position without a graph")
        candidates = [vertex for vertex in self.graph.vertices() if vertex != blocked]
        if not candidates:
            raise RuntimeError("no valid bot start positions")
        if self.bot_kind is BotKind.RANDOM:
        ...
        if role is PlayerRole.COP:
            ...
            scores = {
                vertex: (
                    self.graph.shortest_distance(blocked, vertex),
                    len(self.graph.neighbors(vertex)),
                )
                for vertex in candidates
            }
        ...
        best_vertices = [vertex for vertex, score in scores.items() if score == best_score]
        return self.rng.choice(best_vertices)
